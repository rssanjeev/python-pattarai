...
def process_calls(calls):
    ...
    for call in calls:
        ...
        while start < end:
            ...
            # Fix: Track ongoing calls for ALL intermediate days, not just the last one
            if start < end:
                ongoing_calls[customer_id][current_day].add(call_id)
    ...
    for customer_id, days in customer_calls.items():
        ...
        for date, call_list in sorted(days.items()):  # Process dates in order
            ...
            # ongoing_calls[customer_id][date].clear()
            # print(ongoing_calls)
