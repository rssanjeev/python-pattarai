...
def process_calls(calls):
    ...
    for call in calls:
        ...
        while start < end:
            ...
            # Ensure multi-day calls persist on each day they are active
            if day_end < end:
    ...
    for customer_id, days in customer_calls.items():
        ...
        for date, call_list in sorted(days.items()):  # Process dates in order
            ...
            # Include previous day's ongoing calls before sorting events
            active_calls = set(active_ongoing_calls)  
            ...
            # Start max concurrency with ongoing calls
            max_concurrent = len(active_calls)
            max_timestamp = 0
            max_call_ids = set(active_calls)

            # Sweep line algorithm to track peak concurrency
            for timestamp, event_type, call_id in events:
                if event_type == 1:  # Call starts
            ...
            # Store result for this customer and date
            if max_concurrent > 0:
                results.append({
            ...
            # Ensure only calls continuing into the next day are tracked
            active_ongoing_calls = ongoing_calls[customer_id][date].copy()
            ongoing_calls[customer_id][date].clear()  # Reset after copying to avoid retaining old calls
