...
# POST_URL = f"https://candidate.hubteam.com/candidateTest/v3/problem/result?userKey={USER_KEY}"
...
def process_calls(calls):
    ...
    Returns a structured result that can be sent to the API.
    """
    ...
    for call in calls:
        ...
        while start < end:
            ...
            # Store the ongoing call reference for the next day
            if day_end < end:
                ongoing_calls[customer_id][next_day].add(call_id)  # Fix: Track ongoing calls
    ...
    for customer_id, days in customer_calls.items():
        ...
        for date, call_list in sorted(days.items()):  # Process dates in order
            # Fix: Include calls continuing from previous days
            active_calls = active_ongoing_calls.copy()

            # Sort events (start or end of a call) for the sweep line algorithm
            events = []
            for start, end, call_id in call_list:
                events.append((start, 1, call_id))  # Call start
                events.append((end, -1, call_id))   # Call end

            # Sort by timestamp (break ties by processing start first)
            events.sort()

            max_concurrent = len(active_calls)  # Fix: Start with ongoing calls
            max_timestamp = 0
            ...
            # Fix: Update ongoing calls for the next day
...
if __name__ == "__main__":
    try:
        ...
        # send_results(processed_results)
