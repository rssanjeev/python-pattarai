...
TEST_URL = "https://candidate.hubteam.com/candidateTest/v3/problem/test-dataset-answer?userKey=98ddfac749640524d2b6c2260811"

def fetch_solution_data():
    """Fetches call data from the given API endpoint."""
    response = requests.get(TEST_URL)
    if response.status_code == 200:
        print(response.json()) #["callRecords"]
    else:
...
def process_calls(calls):
    ...
    for customer_id, days in customer_calls.items():
        for date, call_list in days.items():
            ...
            # Sort by timestamp (break ties by start first, then end)
            events.sort()
