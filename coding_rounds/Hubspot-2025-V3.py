...
def process_calls(calls):
    ...
    for call in calls:
        ...
        while start < end:
            ...
            if day_end < end:
                ongoing_calls[customer_id][next_day].add(call_id)  # Fix: Track ongoing calls for the next day
    ...
    for customer_id, days in customer_calls.items():
        ...
        for date, call_list in sorted(days.items()):  # Process dates in order
            # Fix: Include calls continuing from previous days BEFORE sorting events
            active_calls = set(active_ongoing_calls)  # Clone ongoing calls at the start of the day
