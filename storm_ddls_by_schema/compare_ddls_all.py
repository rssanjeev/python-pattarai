...
def parse_ddl_columns(ddl_content: str, table_name: str = "", debug: bool = False) -> List[Column]:
    """
    ...
    for col_def in column_definitions:
        ...
        if "MATERIALIZED VIEW" in ddl_content.upper():
            ...
            if col_match:
                ...
                pass  # Failed to parse MV column
        else:
            ...
            if col_match:
                ...
                pass  # Failed to parse table column
    ...
    for table_name in target_tables:
        ...
        for schema_name in sorted(all_schemas):
            ...
            if ddl_schema and sql_schema:
                ...
                print(f"     📊 DDL ({len(ddl_columns)}): {', '.join(sorted(ddl_columns))}")
                print(f"     📁 STORM ({len(sql_columns)}): {', '.join(sorted(sql_columns))}")
...
if __name__ == "__main__":
    ...
    print(f"🎯 Analyzing {len(target_tables)} tables")
