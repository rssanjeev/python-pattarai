...
TEST_RESULT_URL="https://candidate.hubteam.com/candidateTest/v3/problem/test-dataset-answer?userKey=98ddfac749640524d2b6c2260811"
...
def process_calls(calls):
    ...
    for call in calls:
        ...
        while start < end:
            ...
            if next_day == timestamp_to_utc_date(end):
    ...
    for customer_id, days in customer_calls.items():
        ...
        for date, call_list in sorted(days.items()):  # Process dates in order
            ...
            # Fix: Include ongoing calls in active_calls BEFORE processing events
            active_calls = set(active_ongoing_calls)  # Ensure previous day's calls are included
            # print(active_calls)
            ...
            # Fix: Ensure calls continuing into the next day are properly tracked
