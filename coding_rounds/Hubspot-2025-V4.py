...
def process_calls(calls):
    ...
    for customer_id, days in customer_calls.items():
        ...
        for date, call_list in sorted(days.items()):  # Process dates in order
            # Fix: Include calls continuing from previous days in event list before sorting
            events = []
            ...
            # Fix: Include ongoing calls in active set before processing
            active_calls = set(active_ongoing_calls)  # Clone previous day's active calls
